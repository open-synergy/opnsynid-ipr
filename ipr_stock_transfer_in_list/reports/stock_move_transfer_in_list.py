...
class StockMoveTransferInList(models.Model):
    _name = "stock.move_transfer_in_list"
    ...
    def _select_field(self):
        _super = super(StockMoveTransferInList, self)
        new_query = """
            C.warehouse_id AS source_dest_loc_wh_id,
            E.warehouse_id AS source_loc_wh_id
        """
        select_field_str = (
    ...
    def _join(self):
        _super = super(StockMoveTransferInList, self)
        new_query = """
            JOIN stock_move AS D ON D.move_dest_id=A.id
    ...
    def _get_subtype(self, cr):
        query = """
            ...
            AND     name = 'inter_warehouse_in_subtype'
        """
        cr.execute(query)
